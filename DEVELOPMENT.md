...
**Phase 6b — pipeline + derived quantities — 🟡 core pipeline done, see [docs/phase6_remaining_work.md](docs/phase6_remaining_work.md):** `HaloExtractStage`/`TreeExtractStage`/`CrossMatchStage` (`pipeline.py`) and every stage in `derived.py` (`HaloPropertiesStage`, `OrbitalPropertiesStage`, `StarFormationHistoryStage`, `HostEnvironmentStage`, `EnvironmentStage`, `ObservabilityStage`, `DorchaSpecificStage`) are implemented, as are both `GalaxyBackend`s (`SharkGalaxyBackend`, including optional real-FSPS photometry via `compute_photometry=True`; `HydroGalaxyBackend`) in `backends.py`. Remaining gaps are fields that need infrastructure this codebase doesn't have yet (particle tagging, a cosmic-web classifier, full progenitor-list tree parsing, common-grid mass accretion) or a physical modelling choice nobody has pinned down (`OrbitalEnergy`/`OrbitalPeriod`/`TidalTrackClass`, survey selection functions, structural/light profiles for `HalfLightRadius`) — each documented field-by-field in the owning stage's docstring. The base `ExtractStage.run()` stub is deliberately abstract (only its two subclasses are meant to be instantiated), not an open item.
