...
**Phase 2 — trees + galaxies + epoch models**
...
**Phase 3 — Simulation + cross-matching**
...
Issues noticed during the audit, independent of the new layer:

- `analysistools/__init__.py`: `__all__` lists `HaloCatalogue` (never imported — the class is `HaloTools`) and `analysis_tools` (no such module). *(Fixed alongside this document.)*
- `halo_tools.orig.py` — superseded copy; large commented-out block at the bottom of `halo_tools.py`. Remove once confident.
- `merger_tree_subfind_tools.py` — 10-line stub; fold into `treeio_subfind.py` or delete.
- `galaxy_tools.py` — duplicated `self.vel` read (bug); superseded by `SharkModel` (see §3.7); not currently exported in `__init__.py`.
- `snapshot_tools.py` mixes `CamelCase` (`LoadParticlesByType`, `UnitConversion`, `ParticleOffsetsByType`) and `snake_case` methods — new layer exposes `snake_case` only.
- No test suite. Phase 1 introduces `pytest` + the small files already in `data/` as fixtures; `shark/photometry/test_photometry.py` exists as a starting pattern.
- No `pyproject.toml` — `setup.py` is minimal; worth modernising when tests land (entry for `pytest`, optional extras: `[fsps]`, `[plotting]`).
