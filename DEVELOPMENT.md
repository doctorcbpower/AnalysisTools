...
**Phase 6b — pipeline + derived quantities — 🟡 mostly done, see [docs/phase6_remaining_work.md](docs/phase6_remaining_work.md):** `HaloExtractStage`/`TreeExtractStage`/`CrossMatchStage` (`pipeline.py`) and every stage in `derived.py` (`HaloPropertiesStage`, `OrbitalPropertiesStage`, `StarFormationHistoryStage`, `HostEnvironmentStage`, `EnvironmentStage`, `ObservabilityStage`, `DorchaSpecificStage`) are implemented, as are both `GalaxyBackend`s (`SharkGalaxyBackend`, `HydroGalaxyBackend`) in `backends.py`. Remaining gaps are fields that need infrastructure this codebase doesn't have yet (particle tagging, a cosmic-web classifier, full progenitor-list tree parsing, common-grid mass accretion) or a physical modelling choice nobody has pinned down (`OrbitalEnergy`/`OrbitalPeriod`/`TidalTrackClass`, survey selection functions) — each documented field-by-field in the owning stage's docstring. The base `ExtractStage.run()` stub is deliberately abstract (only its two subclasses are meant to be instantiated), not an open item. `CatalogueBuilder`'s three methods (`_load_config`/`run`/`run_from_stage`) are still fully unimplemented — orchestration is a 6c-adjacent item, see the doc above.
**Phase 6c — validation + webreport — 🟡 SchemaValidator done, rest 🔲 not started:** `validation.SchemaValidator.check()` is implemented (schema presence/dtype checks plus a little-h/comoving-vs-declared-units cross-check, pulled forward from 6c to close the "unit reconciliation isn't implemented" gap flagged throughout `derived.py`/`backends.py`). `IntegrityValidator`/`PhysicalValidator`/`ValidationReport.to_json()`, `pipeline.QualityControlStage`/`WriteStage`, and `webreport.WebReport.build` still raise `NotImplementedError`. See [docs/phase6_remaining_work.md](docs/phase6_remaining_work.md) for the full breakdown and suggested order.
