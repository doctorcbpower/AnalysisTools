...
**Phase 6 — Master science catalogues (`analysistools.catalogue`)** — 🔲 proposed 2026-08-06
Write-side pipeline that builds one flat, versioned, self-describing HDF5 catalogue per project (one row per satellite/subhalo; expensive quantities computed once and stored, never recomputed in analysis notebooks), plus a read-side `CatalogueDataset` adapter (`kind="satellites"`) so the output re-enters the unified interface and every existing `api/plotting.py` function works on it unchanged. Full per-field schema (groups, dtypes, units, provenance, derived-quantity catalogue, validation checks, pitfalls) is specified in [docs/dorcha_master_catalogue_design.md](docs/dorcha_master_catalogue_design.md); the mapping onto this codebase's actual conventions (no astropy — plain-string units in `meta["units"]`, `Simulation`/`Epoch` as the Extract+cross-match layer, snake_case, lazy loading) is in [docs/master_catalogue.md](docs/master_catalogue.md).
Key design decision: galaxy properties are behind a pluggable `GalaxyBackend` (`SharkGalaxyBackend` wrapping `epoch.galaxies_in_halo`, `HydroGalaxyBackend` wrapping `epoch.particles_in_halo(species="star")` for Arepo-Solas/SWIFT hydro runs with no SAM step) so the *same* schema and pipeline serve both SHARK-on-DMO projects (Dorcha) and future hydrodynamic suites — a project is a YAML config selecting the backend and derived-quantity stages, not a fork of the code. A static, self-contained HTML report generator (`catalogue/webreport.py`, built on the existing `plotly` dependency) covers "shareable results" (GitHub Pages / supplementary material / Zenodo attachment) without standing up a server.
Status: `analysistools/catalogue/` package skeleton landed 2026-08-06.
**Phase 6a — schema + read-side — ✅ landed 2026-08-06:** full field table (86 fields across 13 groups) transcribed from the design doc into `analysistools/catalogue/configs/schema_v1.0.yaml` (package data); `CatalogueSchema.from_yaml`/`to_yaml`/`validate_columns` implemented (`schema.py`); `default_schema()`/`available_schema_versions()` helpers; new `catalogue` extra (`pyyaml`) since YAML I/O wasn't previously a core dependency. `CatalogueDataset` (read-side adapter, `kind="satellites"`) was implemented alongside this in the initial scaffold and confirmed working end-to-end against a synthetic file. Example project config at `configs/dorcha.yaml`. 14 new tests in `tests/test_catalogue_schema.py` (55 total, all passing).
**Phase 6b — pipeline + derived quantities — 🟡 core pipeline done, see [docs/phase6_remaining_work.md](docs/phase6_remaining_work.md):** `HaloExtractStage`/`TreeExtractStage`/`CrossMatchStage` (`pipeline.py`) and every stage in `derived.py` (`HaloPropertiesStage`, `OrbitalPropertiesStage`, `StarFormationHistoryStage`, `HostEnvironmentStage`, `EnvironmentStage`, `ObservabilityStage`, `DorchaSpecificStage`) are implemented, as are both `GalaxyBackend`s (`SharkGalaxyBackend`, including optional real-FSPS photometry via `compute_photometry=True`; `HydroGalaxyBackend`) in `backends.py`. Full progenitor-list tree parsing (`NumberOfMergers`) now works for TreeFrog walkable trees (`treeio_treefrog.get_progenitors_treefrog`, inverting the raw file's `Head`/`HeadSnap` pointers) and SubFind-HBT trees (`treeio_subfind.get_progenitors_subfind`, walking the raw file's native `TreeFirstProgenitor`/`TreeNextProgenitor` linked list — both dispatched via `MergerTreeTools.get_progenitors`/`count_mergers`) — still unavailable for TreeFrog's non-walkable "full tree" flavour (no reverse pointer to invert). Remaining gaps are fields that need infrastructure this codebase doesn't have yet (particle tagging, a cosmic-web classifier, common-grid mass accretion) or a physical modelling choice nobody has pinned down (`OrbitalEnergy`/`OrbitalPeriod`/`TidalTrackClass`, survey selection functions, structural/light profiles for `HalfLightRadius`) — each documented field-by-field in the owning stage's docstring. The base `ExtractStage.run()` stub is deliberately abstract (only its two subclasses are meant to be instantiated), not an open item.
**Phase 6c — validation + webreport + orchestration — ✅ landed 2026-08-09:** `validation.py`'s `SchemaValidator`/`IntegrityValidator`/`PhysicalValidator` and `ValidationReport.to_json()`, `pipeline.QualityControlStage`/`WriteStage`/`CatalogueBuilder` (`_load_config`/`run`/`run_from_stage`, plus `HostJob`/`_concatenate_contexts` for multi-host builds), and `webreport.WebReport.build()` are all implemented and tested, including a full `CatalogueBuilder.run()` round trip against the bundled real data (build → validate → write `.h5` → read back). An end-to-end demo notebook (`notebooks/DorchaCatalogueEndToEnd.ipynb`, generated by `analysistools/catalogue/_build_endtoend_notebook.py`) exercises the whole pipeline against real Dorcha data once its CONFIG cell is filled in. See [docs/phase6_remaining_work.md](docs/phase6_remaining_work.md) for the remaining 6b deferred-field list and two known follow-up items (a snapshot-writer indexing bug found in passing, not fixed; `CatalogueBuilder`'s `metadata:` config hook for provenance attributes).

Each phase = one PR-sized change set with tests. Estimated sizes: P1 and P4 are the substantial ones; P2/P3/P5 are mostly glue; P6 is the largest single phase to date (schema + pipeline + validation + a new Dataset kind) and should probably be sub-phased (6a schema/read-side, 6b pipeline/derive, 6c validation/webreport) when implementation starts.
