...
**Phase 6b — pipeline + derived quantities — 🔲 not started:** `pipeline.py`'s `ExtractStage`/`CrossMatchStage`/`CatalogueBuilder.run` and every class in `derived.py` still raise `NotImplementedError`; also needs the actual `GalaxyBackend.galaxy_properties` bodies in `backends.py`. This is the substantial remaining piece — it's where `Epoch.particles_in_halo`/`galaxies_in_halo`/`track_of` and `MergerTreeTools.analyse_orbit` actually get called against real Dorcha data.
**Phase 6c — validation + webreport — 🔲 not started:** `validation.py`'s three `Validator` subclasses and `webreport.py`'s `WebReport.build` still raise `NotImplementedError`.
